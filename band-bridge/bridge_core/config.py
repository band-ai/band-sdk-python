...
in :class:`BridgeConfig.agents` has its own Band WS subscription and its
own :class:`Target` describing where to forward events.

Loaded from a single ``BAND_BRIDGE_AGENTS`` JSON env var, e.g.::
...
class AgentConfig(BaseModel):
    ...
    The bridge opens a Band WS subscription as ``agent_id`` (authed with
...
class BridgeConfig(BaseModel):
    ...
    def from_env(cls) -> BridgeConfig:
        """Load configuration from ``BAND_BRIDGE_AGENTS`` (JSON) plus optional overrides.

        Raises:
            ValueError: If ``BAND_BRIDGE_AGENTS`` is missing or invalid.
        """
        agents_json = os.environ.get("BAND_BRIDGE_AGENTS")
        if not agents_json or not agents_json.strip():
            raise ValueError(
                "BAND_BRIDGE_AGENTS environment variable is required "
                "(JSON list of agent configs)"
            )
        try:
            agents_data = json.loads(agents_json)
        except json.JSONDecodeError as e:
            raise ValueError(f"BAND_BRIDGE_AGENTS is not valid JSON: {e}") from e
        if not isinstance(agents_data, list):
            raise ValueError("BAND_BRIDGE_AGENTS must be a JSON array")

        kwargs: dict[str, Any] = {"agents": agents_data}

        if "BAND_WS_URL" in os.environ:
            kwargs["ws_url"] = os.environ["BAND_WS_URL"]
        if "BAND_REST_URL" in os.environ:
            kwargs["rest_url"] = os.environ["BAND_REST_URL"]
