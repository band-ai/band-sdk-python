...
async def test_a_relay_that_cannot_take_a_role_still_starts(
    ...
    """No role must not mean no room view.

    Every read path works over REST, and the same platform refusal arriving a
    minute later only degrades this process to polling — so failing the launch
    on it would turn a degraded room view into none at all, which the user
    cannot repair without restarting Desktop.
    """
    ...
    with monkeypatch.context() as scoped:
        ...
        await relay.start()
        try:
            assert not relay.status.live, "a roleless relay must not claim liveness"
            assert relay.status.warning, "the agent is told events arrive by REST"
            assert str(refused) in str(relay.status.last_error)
        finally:
            await relay.stop()


async def test_a_failing_teardown_still_gives_back_the_listener(
    relays: RelayHarness,
) -> None:
    """Leadership ends by releasing three things; one failure cannot eat the rest.

    The WebSocket is the likeliest to fail here — leadership often ends
    *because* it broke. If that skipped the unix server, the sibling retrying
    for the lock would connect to a listener with no leader behind it, call
    itself a follower, and never receive another event.
    """
    relay = relays.build()
    relay._directory.mkdir(mode=0o700, parents=True, exist_ok=True)
    leading = asyncio.create_task(relay._lead())
    await wait_until(lambda: relay.status.role == "leader")
    assert relay._socket_path.exists()

    presence, link = relays.presences[0], relays.links[0]
    presence.stop.side_effect = RuntimeError("websocket already gone")
    link.supersede()
    await presence.on_disconnected()

    with pytest.raises(RuntimeError, match="websocket already gone"):
        await leading

    assert not relay._socket_path.exists(), "the leader's socket path was left behind"
    assert link.disconnect.await_count == 1
