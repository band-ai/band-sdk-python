...
const SCENARIOS = {
  ...
  /** A quiet tick sheds the roster; the view must keep showing the cached one. */
  async quietTickKeepsRoster(source) {
    const roster = [{ id: "peer-1", name: "Jerry", type: "Agent" }];
    const view = await open(source, "room-a", {
      ...transcript("room-a", [], "2026-01-01T00:00:01Z"),
      participants: roster
    });
    ...
    await view.settle();
    const room = view.contextUpdates().at(-1).params.structuredContent.band_room;

    return { participants: room.participants.map(item => item.name) };
  },
