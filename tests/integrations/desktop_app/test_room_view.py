...
class TestQuietTick:
    def test_a_quiet_tick_does_not_empty_the_roster(
        self, behaviour: dict[str, Any]
    ) -> None:
        """``RoomEvent.tick`` sheds the roster as ``[]``, meaning "unchanged".

        Dropping it instead leaves the model with a room full of nobody for
        as long as the room stays quiet, so it cannot address a peer.
        """
        result = behaviour["quietTickKeepsRoster"]

        assert result["participants"] == ["Jerry"]
