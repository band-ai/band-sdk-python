...
from band.core.deprecation import BandDeprecationWarning
...
class TestA2AGatewayAdapterOnStarted:
    ...
    @pytest.mark.asyncio
    async def test_on_started_defers_http_when_manage_http_server_false(self) -> None:
        """Foreground gateway path prepares the server without starting it."""
        adapter = A2AGatewayAdapter(manage_http_server=False)
        ...
        with patch(
            ...
            assert adapter.http_server is mock_server

    @pytest.mark.asyncio
    async def test_on_started_warns_when_adapter_starts_http(self) -> None:
        """Legacy Agent.run() path emits a deprecation warning."""
        adapter = A2AGatewayAdapter()
        ...
        with patch(
            ...
            with pytest.warns(
                ...
                match="A2AGateway\\(agent=\\.\\.\\.\\)\\.serve",
            ):
                await adapter.on_started("Gateway", "A2A Gateway Agent")

    @pytest.mark.asyncio
    async def test_cleanup_all_stops_http_server(self) -> None:
        adapter = A2AGatewayAdapter()
        mock_server = MagicMock()
