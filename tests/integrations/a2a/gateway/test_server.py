...
@pytest.mark.asyncio
async def test_serve_and_stop_hand_the_port_back() -> None:
    """The gateway restarts on a fixed port, so stop() must release it.

    Real server, real port: a stop that only signals uvicorn leaves the
    socket bound, and the second pass below dies with EADDRINUSE — invisible
    to any test that fakes uvicorn away.
    """
    import asyncio
    import socket
    ...
    async def until_listening() -> None:
        for _ in range(250):
            with socket.socket() as probe:
                if probe.connect_ex(("127.0.0.1", port)) == 0:
                    return
            await asyncio.sleep(0.02)
        raise AssertionError(f"gateway never listened on {port}")
    ...
    for _ in range(2):
        server = GatewayServer(
            ...
            port=port,
            on_request=None,  # type: ignore[arg-type]
        )
        task = asyncio.create_task(server.serve())
        await until_listening()
