...
class L4ReplayGuardInput(BaseModel):
...
class _NoReplayAnthropicAdapter(AnthropicAdapter):
    def __init__(self, *, calls: list[L4ReplayGuardInput]) -> None:
        async def l4_replay_guard(args: L4ReplayGuardInput) -> dict[str, str]:
            calls.append(args)
            return {"replayed": args.code}

        super().__init__(
            provider_key="test-provider-key",
            additional_tools=[(L4ReplayGuardInput, l4_replay_guard)],
        )
        self._responses = [
            _ScriptedResponse(
