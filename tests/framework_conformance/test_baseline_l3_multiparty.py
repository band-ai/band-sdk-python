...
def _participants_with_greeter() -> list[dict]:
    return [
        *canonical_participants(),
        {
...
def _multi_author_history() -> list[dict]:
    return [
        history_message(
...
async def test_l3_multi_author_history_order_and_attribution_reach_adapter_payloads(
    ...
    if captured.supports_speaker_roles:
        user, _ = _token_position(captured, "USER-L3-TURN")
        calc, _ = _token_position(captured, "CALC-L3-TURN")
        greeter, _ = _token_position(captured, "GREETER-L3-TURN")
