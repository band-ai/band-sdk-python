...
``_effective_timeout`` sets each baseline test's hard pytest-timeout. That backstop
...
invariant across the marker forms ``_effective_timeout`` understands.
"""
...
from tests.e2e.baseline.conftest import _TIMEOUT_BACKSTOP_MARGIN_S, _effective_timeout
...
def test_no_marker_adds_the_backstop_margin_over_the_turn_budget() -> None:
    ...
    assert _effective_timeout(_item(None), BASE) == BASE + _TIMEOUT_BACKSTOP_MARGIN_S
...
def test_extra_beyond_the_margin_is_honored() -> None:
    ...
    assert _effective_timeout(_item(_marker(extra=480)), BASE) == BASE + 480
...
def test_extra_below_the_margin_never_drops_under_the_backstop_floor() -> None:
    ...
    assert (
        _effective_timeout(_item(_marker(extra=5)), BASE)
        == BASE + _TIMEOUT_BACKSTOP_MARGIN_S
    )
...
def test_absolute_timeout_is_left_to_pytest_timeout_natively() -> None:
    ...
    assert _effective_timeout(_item(_marker(600)), BASE) is None
