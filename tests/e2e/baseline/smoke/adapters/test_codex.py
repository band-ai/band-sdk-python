...
async def test_codex_thoughts_are_not_placeholders(
    ...
    A completed reply proves the turn ran. Any thought events that *did* land
    must carry real content — never the literal ``(reasoning)`` / ``(plan)``
    placeholders the adapter used to emit for empty summaries. The user message
    uses ``reasoning_joke_instruction`` so ``name == marker`` and the ask itself
    invites reasoning (how a joke might be badly interpreted).
    """
