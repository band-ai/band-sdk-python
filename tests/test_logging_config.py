...
def test_a_format_style_without_a_format_is_rejected() -> None:
    """The default format string is %-style.

    Pairing it with another style reaches dictConfig as a formatter it cannot
    build, which it reports only as "Unable to configure formatter 'console'"
    — no mention of the style that caused it.
    """
    with pytest.raises(ValueError, match="fmt_style requires"):
        build_logging_config(fmt_style=FormatStyle.BRACE)
