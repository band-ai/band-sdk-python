...
`create_agents.py` carries PEP 723 metadata, so `uv run` fetches the SDK on the
fly — no separate `pip install` needed. It loads this directory's `.env`, so it
registers the agents on the **same** `BAND_REST_URL` Compose connects to (set it
once in `.env`).
...
- **`httpx.ConnectError: [Errno -2] Name or service not known` (crash loop)**:
  ...
  host. `create_agents.py` reads the same `.env`, so it registers the agents on
  whatever `BAND_REST_URL` you set there (default `https://app.band.ai`) — fix
  the value in `.env`, re-run `uv run create_agents.py` if the platform changed,
