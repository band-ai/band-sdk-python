...
async def setup_memory_agent(server: p.Server, tools: list) -> p.Agent:
    ...
    await agent.create_guideline(
        ...
        action=(
            ...
            "scope='organization', and include a brief thought explaining why "
            "the memory is durable. Briefly acknowledge what you saved."
        ),
        tools=tools,
    )
...
async def main() -> None:
    ...
    # Parlant uses OpenAI for fast, tool-capable inference. Requires OPENAI_API_KEY.
