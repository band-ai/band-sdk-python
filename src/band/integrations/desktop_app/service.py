...
from band.integrations.desktop_app.tools import MonitorCaller
...
class RoomTranscriptService:
    ...
    def unannounced_rooms(
        self, current_chat_id: str, *, caller: MonitorCaller
    ) -> list[str]:
        ...
        Reading discharges the notice, so only the agent's own tick may read
        it. The view's display loop ticks every few seconds and its summary
        never reaches the model, so letting it read would consume the
        announcement the agent was owed.
        """
        if caller is not MonitorCaller.MODEL:
            return []
        fresh = [
            room
