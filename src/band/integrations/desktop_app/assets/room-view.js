...
(() => {
  ...
  /**
   * The value a shed field keeps: what the payload carries, or the cache.
   *
   * A quiet tick sheds the fields the view already holds (`RoomEvent.tick`
   * server-side) by sending them empty — which means "unchanged", never "now
   * empty". Emptiness is the whole signal, so it is spelled once here rather
   * than left to each field's truthiness: `""` is falsy but `[]` is not.
   * @template T
   * @param {T | undefined | null} carried
   * @param {T} cached
   * @returns {T}
   */
  function kept(carried, cached) {
    return carried == null || carried.length === 0 ? cached : carried;
  }
  ...
  function absorbState(payload) {
    ...
    participants = kept(payload.participants, participants);
    ...
    roleBriefing = kept(payload.role_briefing, roleBriefing);
