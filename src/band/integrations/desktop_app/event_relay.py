...
class RelayTuning(BandSettings):
...
class DesktopRoomEventRelay:
    ...
    async def _lead(self) -> None:
        ...
        async def release_listener(server: asyncio.AbstractServer) -> None:
        ...
        # Every release is registered the moment the thing it releases
        # exists, and the stack runs all of them however leadership ends. A
        # teardown written as one sequence of awaits abandons the rest at the
        # first failure: a WebSocket that errors exactly as leadership ends
        # would leave the unix server bound and its path in place, and the
        # sibling retrying for the lock would attach to that dead listener,
        # call itself a follower, and receive nothing ever again.
        async with AsyncExitStack() as leadership:
            server = await asyncio.start_unix_server(accept, path=self._socket_path)
            leadership.push_async_callback(release_listener, server)
            ...
            leadership.push_async_callback(link.disconnect)
            ...
            leadership.push_async_callback(presence.stop)
            # Registered last so it unwinds first: releasing the listener
            # waits on every follower handler, and until then a status read
            # would still claim this relay has a live WebSocket.
            leadership.callback(mark_offline)
