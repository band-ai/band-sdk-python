...
- **mcp__band__band_store_memory** - Persist important information
- **mcp__band__band_list_memories** - List stored memories
- **mcp__band__band_get_memory** - Retrieve a specific memory
- **mcp__band__band_supersede_memory** - Mark outdated memories
- **mcp__band__band_archive_memory** - Archive memories
"""
...
- **mcp__band__band_list_contacts** - List your contacts
- **mcp__band__band_add_contact** - Send a contact request
- **mcp__band__band_remove_contact** - Remove a contact
- **mcp__band__band_list_contact_requests** - List pending requests
- **mcp__band__band_respond_contact_request** - Approve/reject requests
"""
...
**You MUST use `mcp__band__band_send_message` to send ANY response.**
Plain text responses will NOT be delivered. Always call the tool.

### MCP Tools (band server)
...
```
1. Use mcp__band__band_lookup_peers to find available agents
2. Use mcp__band__band_add_participant to add the agent
...
"""
    ...
    return SystemPromptPreset(  # type: ignore[not-callable]
        type="preset", preset="claude_code", append=band_instructions
    )
