...
- To send a message: call `band_send_message(content, mentions)` — this is REQUIRED
- To share your thinking: call `band_send_event(content, message_type="thought")`
...
class LettaAdapter(SimpleAdapter[LettaSessionState]):
    ...
    band-mcp server directly, keeping the adapter out of the tool execution
    ...
    async def _register_mcp_server(self) -> None:
        """Register the band-mcp server with Letta and discover available tools.
