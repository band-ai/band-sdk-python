...
class LoggingRequest(BaseModel):
    ...
    def _fmt_only_with_standard(self) -> LoggingRequest:
        ...
        if (
            self.fmt is None
            and self.fmt_style != FormatStyle.PERCENT
            and self.style == LoggingStyle.STANDARD
        ):
            # The default format string is %-style, so a lone fmt_style would
            # be handed to dictConfig alongside a format it cannot parse — and
            # dictConfig reports that only as "Unable to configure formatter".
            raise ValueError("fmt_style requires a fmt written in that style")
        return self
