...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="API key for authentication (env: BAND_API_KEY). Prefer env var over CLI to avoid exposing the key in process listings.",
    )
    parser.add_argument(
        "--rest-url",
        default=os.environ.get("BAND_REST_URL", DEFAULT_REST_URL),
        help=f"Band REST API URL (env: BAND_REST_URL, default: {DEFAULT_REST_URL})",
    )
    parser.add_argument(
        "--auth-mode",
        choices=["agent", "user"],
        default=os.environ.get("BAND_AUTH_MODE", "agent"),
        help="Authentication mode: 'agent' or 'user' (env: BAND_AUTH_MODE, default: agent)",
    )
    parser.add_argument(
        "--target-handle",
        default=os.environ.get("BAND_TARGET_HANDLE"),
        help="Handle of the target agent, e.g. @owner/agent-name (env: BAND_TARGET_HANDLE)",
    )
    parser.add_argument(
        "--message",
        default=os.environ.get("BAND_MESSAGE"),
        help="Message to send to the target agent (env: BAND_MESSAGE)",
    )
    parser.add_argument(
        "--timeout",
        type=int,
        default=int(os.environ.get("BAND_TRIGGER_TIMEOUT", str(DEFAULT_TIMEOUT))),
        help=f"Timeout in seconds for the entire operation (env: BAND_TRIGGER_TIMEOUT, default: {DEFAULT_TIMEOUT})",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
...
async def run(args: argparse.Namespace) -> str:
    ...
    """
    if not args.api_key:
        raise ValueError("API key is required. Provide --api-key or set BAND_API_KEY.")
    if not args.target_handle:
        raise ValueError(
            "Target handle is required. "
            "Provide --target-handle or set BAND_TARGET_HANDLE."
        )
    if not args.message:
        raise ValueError("Message is required. Provide --message or set BAND_MESSAGE.")
