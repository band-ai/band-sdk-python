...
def iter_tool_definitions(
    *,
    ...
    Filtering is applied in order:
      ...
      2. Drop contact tools if *include_contacts* is False (capability gate).
      3. If *include_categories* is set, keep only tools in those categories.
      4. If *include_tools* is set, keep only those specific tools.
      5. If *exclude_tools* is set, remove those tools.
    ...
    # Build the excluded set covering memory/contact gates across both
    # surfaces, then apply user-facing filters on top.
    ...
    # Start with the memory-gate baseline for the user-facing filters
    # (include_tools/exclude_tools/include_categories). These currently
    # apply to the agent-tool name space.
    ...
    # Names that were explicitly removed from the agent-tool baseline by
    # the user-facing filters should also be excluded from the final
    # result, even when iterating across both surfaces.
    user_excluded = baseline - allowed_names
