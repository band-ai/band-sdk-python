...
class CrewAIAdapter(SimpleAdapter[CrewAIMessages]):
    ...
    def __init__(
        ...
        # Fold legacy include/exclude/category constructor params into
        # self.features so build_thenvoi_crewai_tools sees them.
        if (
            include_tools is not None
            or exclude_tools is not None
            or include_categories is not None
        ):
            self.features = AdapterFeatures(
                capabilities=self.features.capabilities,
                emit=self.features.emit,
                include_tools=tuple(include_tools)
                if include_tools is not None
                else self.features.include_tools,
                exclude_tools=tuple(exclude_tools)
                if exclude_tools is not None
                else self.features.exclude_tools,
                include_categories=tuple(include_categories)
                if include_categories is not None
                else self.features.include_categories,
            )
